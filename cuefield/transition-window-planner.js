...
const { scoreExit, scoreCandidatePair } = require('./section-candidates');
...
function exitOptions(analysis, protectedUntil) {
  ...
  return source
    .filter((candidate) => candidate && candidate.role === 'exit' && toNumber(candidate.time) >= protectedUntil)
    .slice()
    .sort((a, b) => scoreExit(b) - scoreExit(a) || toNumber(a.time) - toNumber(b.time))
    .slice(0, 8);
}
...
function grooveContinuity(from, to, exit, entry, diagnostics) {
  ...
  const b = nearbyBar(to, toNumber(entry && entry.playFrom));
  const differences = [
...
function rankWindow({ exit, entry, sectionChoice, recipeCandidate, diagnostics, fromAnalysis, toAnalysis }) {
  const energyContinuity = clamp(diagnostics.energyScore);
  const tempoCompatibility = clamp(diagnostics.bpmScore);
  const groove = grooveContinuity(fromAnalysis, toAnalysis, exit, entry, diagnostics);
  const continuity = (energyContinuity + groove + tempoCompatibility + clamp(diagnostics.bassScore)) / 4;
  const exitRatio = toNumber(exit.exitRatio, toNumber(exit.time) / Math.max(1, toNumber(fromAnalysis.duration)));
...
function startFallback(fromAnalysis, exit, rejected) {
  const entry = { type: 'start', role: 'entry', source: 'fallback', time: 0, playFrom: 0, landingAt: 0, landingType: 'start', confidence: 0.35 };
  const timeline = [
    { t: 0, deck: 'B', op: 'play', at: 0, volume: 0 },
    { t: 0, deck: 'B', op: 'volume', value: 1, duration: 600 },
    { t: 0, deck: 'A', op: 'volume', value: 0, duration: 600 },
  ...
  return {
    ...
    entry,
    sectionChoice: null,
    recipeCandidate: { recipe: 'honest-start-fallback', score: 0, timeline, window: { audibleStart: 0, audibleEnd: 0.6, audibleOverlap: 0.6, preRollDuration: 0, handoffOffset: 0.6, runwayAvailable: true, landingError: 0 } },
    timeline,
    mixStart: round(toNumber(exit && exit.time)),
    handoffAt: round(toNumber(exit && exit.time) + 0.6),
    audibleOverlap: 0.6,
    preRollDuration: 0,
    exitRatio: round(toNumber(exit && exit.time) / Math.max(1, toNumber(fromAnalysis && fromAnalysis.duration))),
...
function chooseTransitionWindow(fromAnalysis = {}, toAnalysis = {}) {
  ...
  exits.forEach((exit) => {
    entries.forEach((entry) => {
      const sectionChoice = scoreCandidatePair(exit, entry, fromAnalysis, toAnalysis, scoreExit, exits);
      const recipePlan = planRecipeCandidates(fromAnalysis, toAnalysis, {
      ...
      (recipePlan.candidates || []).forEach((recipeCandidate) => {
        ...
        const window = rankWindow({ exit, entry, sectionChoice, recipeCandidate, diagnostics: recipePlan.diagnostics || {}, fromAnalysis, toAnalysis });
  ...
  const chosen = candidates[0] || startFallback(fromAnalysis, exits[0], rejected);
